...
const SORT_KEY_STORAGE = "fileListSortKey";
const SORT_ORDER_STORAGE = "fileListSortOrder";
...
const FileList: FC<FileListProps> = ({ showStarredFirst }) => {
    ...
    const initialSortKey: SortKey = (localStorage.getItem(SORT_KEY_STORAGE) as SortKey) || "name";
    const initialSortOrder: SortOrder = (localStorage.getItem(SORT_ORDER_STORAGE) as SortOrder) || "asc";
    ...
    const displayedFiles = useMemo(() => {
        const primarilySorted = sortFiles(files, sortKey, sortOrder);

        if (!showStarredFirst) {
            return primarilySorted;
        }

        const starred = primarilySorted.filter(f => f.starred);
        const unstarred = primarilySorted.filter(f => !f.starred);
    ...
    const handleKey = useCallback(
        ...
        [selected, storage, db, displayedFiles]
    );
