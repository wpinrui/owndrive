...
export const FileRow: FC<Props> = ({ file, selected, onRowClick, onDownload, onDelete, onToggleStar }) => {
    ...
    return (
        <tr className={`file-row ${selected ? "selected" : ""}`} onClick={() => onRowClick(file.id)}>
            ...
            <td className="col-actions">
                <div className="action-buttons">
                    <button className="action-btn download-btn" onClick={e => { e.stopPropagation(); onDownload(file); }} title="Download">⬇</button>
                    {!file.starred && <button className="action-btn delete-btn" onClick={e => { e.stopPropagation(); onDelete(file); }} title="Delete">🗑</button>}
                    <button className="action-btn star-btn" onClick={e => { e.stopPropagation(); onToggleStar(file); }} title={file.starred ? "Unstar" : "Star"}>
                        {file.starred ? "★" : "☆"}
                    </button>
                </div>
            </td>
